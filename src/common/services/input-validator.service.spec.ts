...
describe('InputValidatorService', () => {
  ...
  describe('validate', () => {
    it('should validate correct string input', () => {
      const schema: InputParameter[] = [{ name: 'title', type: 'string', required: true }];
      const result = service.validate(schema, { title: 'Test' });
      expect(result.title).toBe('Test');
    });

    it('should validate correct number input', () => {
      const schema: InputParameter[] = [{ name: 'count', type: 'number', required: true }];
      const result = service.validate(schema, { count: 42 });
      expect(result.count).toBe(42);
    });

    it('should validate correct boolean input', () => {
      const schema: InputParameter[] = [{ name: 'enabled', type: 'boolean', required: true }];
    ...
    it('should validate array input', () => {
      const schema: InputParameter[] = [{ name: 'tags', type: 'array', required: true }];
      const result = service.validate(schema, { tags: ['tag1', 'tag2'] });
      expect(result.tags).toEqual(['tag1', 'tag2']);
    });

    it('should validate object input', () => {
      const schema: InputParameter[] = [{ name: 'metadata', type: 'object', required: true }];
    ...
    it('should throw for string above max length', () => {
      ...
      expect(() => service.validate(schema, { title: 'This is too long' })).toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw for invalid email format', () => {
      ...
      expect(() => service.validate(schema, { email: 'invalid-email' })).toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw for enum without options', () => {
      const schema: InputParameter[] = [{ name: 'status', type: 'enum', required: true }];
