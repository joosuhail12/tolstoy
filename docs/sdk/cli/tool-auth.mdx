...
```
🔧 Configuring API key for tool "tool-123"...
...
```
🔧 Configuring OAuth2 for tool "tool-123"...
...
💡 Next steps:
   1. Users can now run: tolstoy auth login --tool tool-123 --user <userId>
...
tolstoy tool auth api-key -o $ORG_ID -t $TOOL_ID -k $GITHUB_TOKEN
```
...
tolstoy tool auth api-key -o my-org -t tool-123 -k ghp_secret123

# Do this instead
export GITHUB_TOKEN="ghp_secret123"
tolstoy tool auth api-key -o my-org -t tool-123 -k "$GITHUB_TOKEN"
```
...
4. **Verify tool ownership**: Ensure tools are properly associated with your organization
