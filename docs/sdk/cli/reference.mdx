...
```bash
# GitHub API key
tolstoy tool auth api-key -o my-org -t tool-123 -k ghp_xxxxxxxxxxxx

# Custom header
tolstoy tool auth api-key -o my-org -t tool-456 -k secret123 -h "X-API-Key"
```
...
tolstoy auth login -t tool-123 -u user-123 -o my-org

# Manual URL (no browser)
tolstoy auth login -t tool-456 -u user-456 --no-open
```
