...
Output:
```
🚀 Initiating OAuth login for tool "github"
User ID: user-123
Organization: my-org
Tool: github

📱 Opening browser for OAuth authorization...
Login URL: https://api.tolstoy.com/auth/github/login?userId=user-123
...
Output:
```
🚀 Initiating OAuth login for tool "google"
User ID: user-456
Organization: my-org
Tool: google

🔗 OAuth Login URL:
https://api.tolstoy.com/auth/google/login?userId=user-456
...
echo "Starting OAuth login for $USER_ID on $TOOL"

# Start OAuth flow and capture URL
AUTH_OUTPUT=$(tolstoy auth login --tool $TOOL --user $USER_ID --no-open 2>&1)
...
tolstoy auth login --tool github --user new-user-456

# Step 3: Connect Google Drive  
tolstoy auth login --tool google --user new-user-456

# Step 4: Connect Slack
tolstoy auth login --tool slack --user new-user-456
```
...
tolstoy auth login --tool github --user existing-user-789
```
...
```bash
# Authorize for org A
tolstoy auth login --tool github --user user-123 --org org-a

# Authorize for org B  
tolstoy auth login --tool github --user user-123 --org org-b
```
...
```
Error: Tool "github" not configured for OAuth2
```
...
tolstoy auth login --tool github --user user-123 --no-open
```

### Wrong Organization
```bash
# Explicitly specify org (overrides ORG_ID env var)
tolstoy auth login --tool github --user user-123 --org correct-org
```
