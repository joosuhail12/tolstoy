...
The system determines which authentication configuration to use by extracting the tool name from step configuration, then looking up the corresponding tool ID:

### 1. Explicit Tool Name
If the step config includes a `toolName` property, it's used to look up the tool ID:
...
```
This automatically resolves to the tool with name "GitHub" and uses its tool ID for authentication configuration lookup.
...
interface SandboxExecutionContext {
  ...
  authHeaders?: Record<string, string>; // 👈 Added auth headers
}
```
...
const response = await fetch(url, {
  ...
  headers: {
    'Content-Type': 'application/json',
    ...configHeaders,
    ...authHeaders  // 👈 Injected authentication
  },
...
```json
{
  "type": "http_request",
  "config": {
    "toolName": "Slack",  // 👈 Explicit tool name
...
{
  "orgId": "org_12345",
  "userId": "user_67890",  // 👈 Required for OAuth2
  "flowId": "flow_abc123",
  "steps": [...] 
}
```
