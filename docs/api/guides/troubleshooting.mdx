...
if [ -z "$TOLSTOY_API_KEY" ]; then
    echo "❌ TOLSTOY_API_KEY environment variable is not set"
    exit 1
fi

echo "✅ API key is present"
echo "🔍 Key format: ${TOLSTOY_API_KEY:0:8}***"
...
if [ "$HTTP_STATUS" = "200" ]; then
    ...
    echo "❌ API key validation failed (HTTP $HTTP_STATUS)"
...
class WebhookDebugger {
  debugSignature(payload, receivedSignature, timestamp, secret) {
    ...
    if (age > 300) { // 5 minutes
      console.log('⚠️  WARNING: Timestamp is older than 5 minutes');
    }
...
class WebhookDebugger:
    def debug_signature(self, payload: str, received_signature: str, timestamp: str, secret: str) -> bool:
        ...
        if age > 300:  # 5 minutes
            print('⚠️  WARNING: Timestamp is older than 5 minutes')
...
class WebhookDeliveryDebugger {
  async checkWebhookEndpoint(url) {
    ...
    try {
      ...
      console.log('❌ POST requests fail:', error.message);
      return false;
    }
  ...
  async simulateWebhookDelivery(url, secret) {
    ...
    for (const eventData of events) {
      ...
      try {
        ...
        console.log(`${eventData.event}: ${response.status} ${response.ok ? '✅' : '❌'}`);
        ...
        console.log(`${eventData.event}: FAILED - ${error.message} ❌`);
      }
    }
  }
}
...
class DatabaseDiagnostics {
  ...
  async testConnectivity() {
    ...
    try {
      ...
      if (error.code === 'ECONNREFUSED') {
        ...
        console.log('💡 Check DNS resolution for database host');
  ...
  async checkConnectionPool() {
    ...
    if (poolStats.waitingCount > 0) {
      ...
      console.log('💡 Consider increasing pool size or optimizing query performance');
    }

    if (poolStats.totalCount >= parseInt(process.env.DATABASE_POOL_SIZE || '20')) {
      console.log('⚠️  WARNING: Connection pool at maximum capacity');
    }
  }
...
<Accordion title="Error Investigation Tools">
...
class DebugLogger {
  ...
  logDatabaseQuery(query, params, duration, rowCount) {
    ...
    if (this.isDebugMode) {
      console.log(`🗄️  Database Query (${duration}ms):`);
  ...
  enableVerboseMode() {
    ...
    console.log('🔊 Verbose debug mode enabled');
...
class DebugLogger:
    ...
    def log_cache_operation(self, operation: str, key: str, hit: bool, duration: float):
        ...
        if self.is_debug_mode and not hit:
            print(f"💾 Cache MISS: {key} ({duration:.2f}ms)")
    ...
    def log_database_query(self, query: str, params: list, duration: float, row_count: int):
        ...
        if self.is_debug_mode:
            print(f"🗄️  Database Query ({duration:.2f}ms):")
    ...
    def log_performance_metric(self, metric: str, value: float, threshold: float):
        ...
        if value > threshold:
            print(f"⚠️  Performance Alert: {metric} = {value} (threshold: {threshold})")
...
class PerformanceProfiler {
  ...
  analyzeProfile(profile) {
    ...
    Object.entries(profile.phases).forEach(([phase, duration]) => {
      const status = duration > 100 ? '🐌' : duration > 50 ? '⚠️' : '✅';
      console.log(`${status} ${phase}: ${duration}ms`);
    });

    if (profile.response) {
      console.log(`📊 Response: ${profile.response.status} (${profile.response.size} bytes)`);
    }
...
class SystematicDebugger {
  ...
  generateDiagnosticReport(diagnostics) {
    ...
    console.log(`\n🏥 Overall Health: ${overallHealth === 'healthy' ? '✅ Healthy' : '❌ Issues Detected'}`);
...
echo "🔍 Tolstoy Integration Diagnostics"
echo "=================================="
...
if curl -f -s https://api.tolstoy.com/v1/health > /dev/null; then
    echo "✅ Tolstoy API is reachable"
else
    echo "❌ Cannot reach Tolstoy API"
fi
...
class CompleteDiagnosticTool:
    ...
    async def run_full_diagnostics(self):
        print("🔍 Tolstoy Integration Diagnostics")
        print("==================================")
    ...
    async def check_environment(self):
        ...
        for var in env_vars:
            ...
            print(f"{'✅' if is_set else '❌'} {var} is {'set' if is_set else 'not set'}")
    ...
    async def check_authentication(self):
        ...
        print(f"🔍 Key environment: {key_env}")
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get('https://api.tolstoy.com/v1/health', headers=headers) as response:
                    if response.ok:
                        ...
                        print(f"❌ API key validation failed (HTTP {response.status})")
                        auth_status = 'invalid'
    ...
    def generate_final_report(self):
        ...
        if not issues_found:
            print("✅ All diagnostics passed - integration appears healthy")
        else:
        ...
        print(f"\n⏱️  Diagnostic completed in {time.time() - self.start_time:.2f} seconds")
...
class ProactiveMonitor {
  ...
  async sendHealthAlerts(alerts) {
    for (const alert of alerts) {
      console.log(`🚨 ${alert.severity.toUpperCase()} ALERT: ${alert.component} - ${alert.message}`);
